...
export default function AdminProducts() {
  ...
  if (products.length === 0) {
    return(
      <div className="text-center text-gray-400 w-[1100px]">
        <NoAdminProducts/>
      </div>
    )
  }
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {products.map(product => (
        <Card key={product.id} className="bg-[#18181b] border border-[#232329] rounded-2xl shadow-sm overflow-hidden flex flex-col justify-between p-0">
          <CardHeader className="p-0 relative h-40 overflow-hidden rounded-t-2xl">
            <Image 
          ...
          <CardContent className="p-4 flex-1 flex flex-col justify-between">
            <div>
              <CardTitle className="text-lg font-semibold text-white mb-1">{product.name}</CardTitle>
              <div className="text-gray-400 text-sm mb-2 line-clamp-8">{product.description}</div>
              <div className="flex justify-between items-center mb-2">
                <div className="text-green-400 font-bold text-lg">${product.price}</div>
            ...
            <div className="flex gap-2 mt-4">
              <Button 
                variant="outline" 
                className="border-gray-600 text-gray-200 hover:bg-gray-800 flex-1" 
